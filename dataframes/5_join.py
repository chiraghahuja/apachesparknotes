...
region_rdd = sc.parallelize([['Central', 'Chicago'], ['West', 'Seattle'], ['East', 'Boston']])


regions = spark.createDataFrame(region_rdd, ['Region', 'Headquarters'])

sales_df.join(regions,on=['Region'],how='left_outer').orderBy('OrderDate').show(4)